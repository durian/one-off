...
'''
Creates the delta's between the histograms on a per truck basis. For P1FW6
...
df_train = pd.read_csv( "P1FW6_20181008.csv",
                        sep=";", dtype={'VEHICL_ID': str},
                        #https://stackoverflow.com/questions/17465045/can-pandas-automatically-recognize-dates
                        parse_dates=['SEND_DATETIME'],
                        #skiprows=lines,
                        nrows=args.rows
)
print( "Ready" )
...
for  the_vehicle_id in the_list:
    ...
    hist_data    = df_train1.iloc[ :,    4:114]
    hist_pre     = df_train1.iloc[ :,    0:  4]
    hist_post    = None #df_train1.iloc[ :,  404:406]
    #
    ...
    for idx in range(0, num_hists):
        ...
        q = list(hist_pre.iloc[idx,:]) + list(hist_delta)  + [delta_days, delta_cells]
